...
It is attached [here](CPA_rawdata.csv)
...
<br/>
The data contains 2241 unique records and 27 unique variables.
...
Microsoft excel is used because it can be considered the best tool for handling small datasets. It is easy to use, shows you a quick glance of how data is organized and stored. It can also help you find duplicate records, perform sorting, filtering, conditional formatting etc. I have also used MYSQL for data manipulation and exploration such as adding and removing features, and summarizing statistics. Tableau is used for creating high quality data visualizations while Rstudio is used for statistical analysis, data analysis and creating visualizations using coding.
...
As I said earlier Microsoft Excel is one of the best tools for handling small datasets with considerable features. It has number of functions which can be really useful for performing quick analysis and gaining general insights. It is a spreadsheet program that is used to save, explore, manipulate and analyse data. 


So, First and the Foremost thing I did was conversion of .csv file to .xls file just so it can be used in Microsoft Excel. After that, I verified that all features has appropriate datatype, so no datatypes related issues occur. Then, I used sort and filter features of MS Excel to look for any missing or incomplete values. Turns out, the feature “ Income “ has some of the fields empty. As it is an important feature of our analysis it cannot be blank. Therefore, I remove the records where the income field was empty. Now, the total rows remained were 2217.
<br/>


The code for the data cleaning and data manipulation process in MYSQL WorkBench can be accessed from [here](CPA_Script.sql),


After the data cleaning and verification process in Microsoft Excel is completed. The data was exported to MYSQL Workbench for further analysis. MySQL is a free, open source RDBMS (relational database management system). It can be communicated using SQL (structured query language). SQL is a language used to access, manipulate and manage contents of a database. It is fast, easy to use, reliable and used in most of the databasement management systems. Now, using SQL in MYSQL WorkBench some new features are added for analysis purpose as shown below.
...
*	It is also observed that features "Z_costContact" and "Z_revenue" repeats the same values in all the rows respectively. This features does not seem to be contributing much to our analysis so it would be better to remove them to reduced redundancy.
*	The feature "Dt_Customer" was dropped along with "Year_birth" as they were not contributing any major insights.
*	In the Education feature there are 5 types of status mentioned "Basic", "2n Cycle", "Graduate", "Master" and "PhD", but the “2n cycle” education status can be considered equivalent to “ Graduate" status so it will be replaced to “Graduate” status. Same goes for "Masters" and "PhD", Both comes under "PostGraduate" Status and similarly "Basic" represents "UnderGraduate" status so it is changed to it. So the only educational status remained are "UnderGraduate","Graduate" and "PostGraduate".
...
*	Lastly, after creating few new features and dropping some old ones, total number of features are 23 and records are 2214.
...
* From this particular chart it can be said that most of the customers are married and has a single kid while least of the customers are single with three kids. This seems to be one of the most interesting information by far.

![Income and gold](https://user-images.githubusercontent.com/30661622/151690834-8d8dc27c-b8f7-4849-9fdb-1d425fcc86da.png)

* In the bar chart of Income vs GoldProducts, it can be observed that customers with salaries between 0k to 7k buys most gold products. whereas, customer whose salary is in the range of 16k to 100k spends just under 250.
...
After looking for trends and patterns from data visualization in Tableau Public. For further analysis, I used Rstudio. Rstudio is an IDE(Itegrated Development Environment) for R, a programming language used for statistical analysis, data analysis and data visualization. R language is an open source, data centric and easily the most preffered language for processing lots of data and manipulating information to find desired insights.

Now, I have a general idea about customers personality but for finding the types of customers, these three features can play major role. 
* Recency: shows number of days since the customers last purchase over the span of last two years.
* Frequency: displays how frequent the customers goes out for shopping in last two years.
